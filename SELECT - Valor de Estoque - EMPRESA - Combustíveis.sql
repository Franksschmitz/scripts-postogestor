...
from (
  select emp.ID_EMPRESA, 
         ...
         coalesce((select mi.QT_ESTOQUE
          ...
          where mi.CH_EXCLUIDO is null and
                ...
                mi.DT_MOVIME <= '2021-07-15 23:59:59.999'
          order by mi.DT_MOVIME desc, mi.ID_SEQ desc
          limit 1), 0.0) as QT_ESTOQUE,
          (select VL_CUSTO from PROC_CUSTOITEM_STATUS(i.ID_ITEM, '2021-07-15 23:59:59.999', l.ID_FILIAL)) as VL_PRECOBASE ,
          (select VL_PRECO from PROC_PRECOVENDA_STATUS(li.ID_ITEM, emp.ID_EMPRESA, '1-1', '2021-07-15 23:59:59')) as VL_PRECOVENDA
  ...
  and (i.CH_COMBUSTIVEL = 'T') and l.ID_FILIAL in ('1') and coalesce(i.CH_KIT, 'F') <> 'T'
) rel_
where ((QT_ESTOQUE > 0))
