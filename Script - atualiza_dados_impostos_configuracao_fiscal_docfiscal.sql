...
BEGIN	    
   ...
   l_indice_aliquota = ''; --indice da aliquota no ecf. Ex: 01
   ...
   SELECT cfi.AL_IMPOSTO,              
          ...
          cfi.ID_NATOPER, 
          cfi.DS_STECF,
          l_indice_aliquota || cfi.DS_STECF || trunc(cfi.AL_IMPOSTO * 100)   
   into  l_al_imposto,
         l_id_cst,
         l_id_natoper,
         l_ds_stecf,
   ...
   loop
      ...
      loop
        update DOCFISCAL_ITEM_IMPOSTO set
            ...
            DS_STECF = l_ds_stecf,
            VL_BASE = CASE WHEN VL_BASE <= 0 THEN l_total_item ELSE VL_BASE END, 
