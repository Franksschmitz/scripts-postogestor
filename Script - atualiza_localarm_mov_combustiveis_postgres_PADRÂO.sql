/* ---------------------------------------------------- ATENÇÃO ------------------------------------------------------ */
/* ESSE SCRIPT DEVE SER RODADO EM CADA EMPRESA DA REDE POIS OS UPDATES EFETUADOS NÃO VÃO REPLICAR PARA OUTRAS EMPRESAS */
/* RODAR PRIMEIRO EM UM BACKUP E CONFERIR SE FICOU CORRETO                                                             */
/* --------------------------------------------------------------------------------------------------------------------*/
SET TERM ^ ;
...
begin
  ...
  l_ID_ITEM_ERRADO = ''; --substituir esse valor
  l_ID_ITEM_CERTO = ''; --substituir esse valor
  
  l_ID_LOCAL_ERRADO = ''; --substituir esse valor
  l_ID_LOCAL_CERTO = ''; --substituir esse valor
  
  l_ID_BICO_ERRADO = ''; --substituir esse valor
  l_ID_BICO_CERTO = ''; --substituir esse valor
  ...
  where ID_LOCALARM = l_ID_LOCAL_ERRADO and
		...
		exists(
		  ...
		  where m.ID_MOVEST = mi.ID_MOVEST and 
				...
				di.ID_DOCFISCAL_ITEM = mi.ID_DOCFISCAL_ITEM and
				ab.ID_BICO = l_ID_BICO_ERRADO
		);  
        
  update ABASTECIMENTO set
    ID_LOCALARM = l_ID_LOCAL_CERTO,
	ID_ITEM = l_ID_ITEM_CERTO,
	ID_BICO = l_ID_BICO_CERTO
  where ID_LOCALARM = l_ID_LOCAL_ERRADO and
        ID_ITEM = l_ID_ITEM_ERRADO and        
		ID_BICO = l_ID_BICO_ERRADO and
        CH_EXCLUIDO IS NULL;
  ...
  where ID_LOCALARM = l_ID_LOCAL_ERRADO and
		...
		exists(
		  ...
		  where ab.ID_ABASTECIMENTO = di.ID_ABASTECIMENTO and 
		        ab.ID_BICO = l_ID_BICO_CERTO
		);  
