 ...
 * cr.VL_SALDO / 100.0)  else    cr.VL_TAXA  end  as VL_TAXA,  
  ...
  where (cr.CH_EXCLUIDO is null)
        and cr.DT_VENCIM between '2021.01.01' and '2021.02.17' and cr.ID_FILIAL = '2' 
        and (select cd.CH_SITUAC  from CONCIVEN_DET cd  
             where cd.ID_CONCIRECPAG = cr.ID_CONCIRECPAG and        
             cd.CH_EXCLUIDO is null  
             limit 1) = 'D'  
) rel_
