...
from (
        select 
            ...
            sum(cast(di.QT_ITEM * coalesce((select VL_CUSTOMED from PROC_CUSTOMEDIO_ITEM_VENDA(di.ID_DOCFISCAL_ITEM)),0.00) as decimal(18,4))) as TOTAL_CUSTO,
            sum(di.VL_CONTABIL) as TOTAL_VENDA,
        ...
        left join ENTIDADE vend on vend.ID_ENTIDADE = di.ID_VENDEDOR
            ...
            and d.CH_OPERAC = 'S' 
            and d.CH_SITUAC = 'F'
            and d.DT_EMISSA between '2021.02.01' and '2021.02.26'
            and d.CH_TIPO <> 'VEN'
